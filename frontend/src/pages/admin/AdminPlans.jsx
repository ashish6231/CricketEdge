...
export default function AdminPlans({ isSuperAdmin }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {plans.map(plan => (
        <div key={plan._id} className="glass-card rounded-2xl p-5">
          {editing === plan._id ? (
            <div className="space-y-3">
              <div className="flex items-center justify-between mb-1">
                <span className="font-bold text-text-primary capitalize">{plan.slug} Plan — Edit</span>
              ...
              <div>
                <label className="text-xs text-text-muted block mb-1">Features (one per line)</label>
                <textarea rows={5} value={form.features}
                  onChange={e => setForm(p => ({ ...p, features: e.target.value }))}
                  className="w-full rounded-xl border border-border px-3 py-2 text-sm outline-none focus:border-primary bg-white resize-none" />
              </div>
